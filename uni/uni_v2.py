...
class V2LiquiditySnapshot:
    ...
    num_lp_tokens: int
    token0: V2PoolToken
    num_token0_underlying: str
    token1: V2PoolToken
    num_token1_underlying: str


def get_pool_token_info(pool_address: str, token_index: int, block_no) -> PoolTokenInfo:
    ...
    token_decimals = contract_call_at_block(interface_address=token_address,
                                            implementation_address=tokem_address,
                                            fn_name='decimals',
                                            fn_args=[],
                                            block_no=block_no,
                                            chain='ethereum')
    ...
    return V2PoolToken(token_index, token_address, token_symbol, int(token_decimal))
...
def get_underlying_balances_address(pool_address: str, wallet_address: str, block_no: int) -> V2LiquiditySnapshot:
    ...
    pool_total_supply_result = contract_call_at_block(interface_address=pool_address,
                                                      ...
                                                      block_no=block_no,
                                                      chain='ethereum')
    pool_total_supply = Decimal(pool_total_supply) / E18
    ...
    reserves_result = contract_call_at_block(interface_address=pool_address,
                                                      ...
                                                      block_no=block_no,
                                                      chain='ethereum')
    token0_reserves = Decimal(reserves_result[0]) / Decimal(10 ** token0.decmials)
    token1_reserves = Decimal(reserves_result[1]) / Decimal(10 ** token0.decmials)

    wallet_share_of_lp = wallet_lp_balance / pool_total_supply
    token0_underlying_lp = token0_reserves * wallet_share_of_lp
    token1_underlying_lp = token0_reserves * wallet_share_of_lp

    return V2LiquiditySnapshot(block_no, wallet_lp_balance, token0, token0_underlying_lp, token1, token1_underlyi)
